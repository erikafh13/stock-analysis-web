...
def run_stock_analysis(
    ...
    # Simpan ke Parquet (untuk session ini)
    save_df(user, "result_stock", result)

    abc_dist = result["ABC"].value_counts().to_dict()

    # Simpan ke Supabase (permanen — tidak hilang saat server restart)
    saved_to_db = save_stock_result(
        username=user,
        df=result,
        dept_filter=dept,
...
def get_stock_result(
    ...
    """
    Ambil hasil analisis stock dengan pagination (DuckDB).
    Jika Parquet tidak ada (server restart), coba restore dari Supabase.
    """
    ...
    if not path.exists():
        logger.info(f"Parquet tidak ada untuk {user}, coba restore dari Supabase...")
        db_row = get_latest_stock_result(user)
        if db_row and db_row.get("result_json"):
            df_restore = pd.DataFrame(db_row["result_json"])
            save_df(user, "result_stock", df_restore)
            logger.info(f"Berhasil restore {len(df_restore)} baris dari Supabase")
        else:
            raise HTTPException(
                404,
                "Belum ada hasil analisis. Jalankan analisis dulu."
            )
...
def get_summary(user: str = Depends(get_current_user)):
    """Ringkasan metrik dari semua data yang sudah dimuat + riwayat dari Supabase."""
    ...
    # ABC distribution dari Parquet (session ini)
    path = parquet_path(user, "result_stock")
    if path.exists():
        con = duckdb.connect()
        ...
        # Fallback: ambil dari Supabase jika Parquet tidak ada
        db_row = get_latest_stock_result(user)
        if db_row and db_row.get("abc_dist"):
            abc_dist_raw = db_row["abc_dist"]
            summary["abc_distribution"] = [
                {"ABC": k, "jumlah": v} for k, v in abc_dist_raw.items()
            ]
            summary["last_analysis"] = {
                "created_at": db_row.get("created_at"),
                "dept_filter": db_row.get("dept_filter"),
                "rows": db_row.get("rows"),
                "bulan_cols": db_row.get("bulan_cols"),
                "source": "supabase",
            }

    # Riwayat analisis dari Supabase
    history = get_analysis_history(user, limit=5)
    if history:
        summary["history"] = [
            {
                "id": h["id"],
                "created_at": h["created_at"],
                "dept_filter": h["dept_filter"],
                "rows": h["rows"],
                "bulan_cols": h["bulan_cols"],
            }
            for h in history
        ]
...
def get_history(
    ...
    """Riwayat semua analisis yang pernah dijalankan (dari Supabase)."""
    history = get_analysis_history(user, limit=limit)
    if not history:
        return {"history": [], "message": "Belum ada riwayat analisis"}
